--[[
	🏝️ ANIMAL CROSSING: NEW HORIZONS LIGHTING & CAMERA SCRIPT
	...
	FEATURES:
	• Warm, dreamy lighting like AC:NH
	• Proper FOV (70 degrees)
	• Soft shadows and bloom
...
print("🏝️ Applying Animal Crossing visual style...")

-- ============================================================================
-- CAMERA SETTINGS (AC:NH style - Enhanced)
-- ============================================================================

-- AC:NH has a distinctive wide, slightly top-down view
camera.FieldOfView = 85  -- Wider FOV for that iconic AC look (was 70)

-- Camera distance (pulled back for better view of surroundings)
player.CameraMaxZoomDistance = 30  -- Can zoom out more
player.CameraMinZoomDistance = 15  -- Minimum distance (no super close-up)

-- Top-down angle for AC:NH isometric-ish feel
humanoid.CameraOffset = Vector3.new(0, 4, 0)  -- Higher camera position

-- Force third person view
player.CameraMode = Enum.CameraMode.Classic

-- Disable first person (AC:NH is always third person)
player.CameraMaxZoomDistance = 30
player.CameraMinZoomDistance = 15  -- Prevents zooming into first person

print("✅ Camera: FOV 85, top-down angle, third-person locked")
...
-- Remove harsh directional light in favor of ambient
-- (AC:NH uses lots of ambient light for that soft look)

-- Adjust workspace properties for better visuals
Workspace.StreamingEnabled = false  -- Keep whole world loaded for better visuals
...
-- Grass and foliage should be brighter
for _, part in pairs(Workspace:GetDescendants()) do
	if part:IsA("BasePart") then
		if part.Material == Enum.Material.Grass then
			-- Make grass brighter and more vibrant
...
print("═══════════════════════════════════════════════════════════════")
print("🎨 ANIMAL CROSSING VISUAL STYLE APPLIED!")
print("═══════════════════════════════════════════════════════════════")
print("📷 Camera: FOV 85° (wide angle), top-down view, third-person locked")
...
print("═══════════════════════════════════════════════════════════════")
print("🎮 Enjoy your cozy island experience! 🏝️")
print("═══════════════════════════════════════════════════════════════")
