...
function ItemBrowser.new()
    ...
    local modulesFolder = ReplicatedStorage.Parent:FindFirstChild("client")
        and ReplicatedStorage.Parent.client:FindFirstChild("Modules")
...
function ItemBrowser:loadAllItems()
    ...
    print(
        "[ItemBrowser] Loaded",
        #itemsWithSprites,
        "total sprite positions (",
        #itemsBySpriteIndex,
        "mapped,",
        #itemsWithSprites - #itemsBySpriteIndex,
        "unmapped)"
    )
end
...
function ItemBrowser:createItemButton(item)
    ...
    local _rightClickConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed or not isMouseOver then
            return
        end
